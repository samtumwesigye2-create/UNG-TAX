...
def test_revenue_operations_exposes_password_recovery():
    ...
    reset_js = Path("static/ops-password-reset.js").read_text(encoding="utf-8")
    assert "forgotPasswordBtn" in html
    assert "resetPanel" in html
    assert "ops-password-reset.js" in html
    assert "/auth/password/request" in reset_js
    assert "/auth/password/confirm" in reset_js
